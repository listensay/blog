...
/**
 * AI 结果的确认弹窗 —— 这个后台的原则是「不偷偷改我的文章」，所以 AI 的产出
 * 一律先摊开给人看，点了「替换原文」才会真的动编辑器里的内容。
 *
 * 三块内容，按「最该先看到的排最前」：
 *
 *  1. **完整性警告**。图片地址、代码块、标题层级被改动了会在最上面报出来。
 *     有 error 级问题时「替换原文」需要多点一次确认 —— 不禁用，因为有时候
 *     你就是想拿它当草稿再手改，但不能让人一路回车点过去。
 *  2. **差异**。行级比对，默认只看改动附近；结构性的东西（标题、代码块、图片）
 *     独占一行，会显示成「没变」，一眼就能确认模型守没守规矩。
 *  3. **结果**。可以直接编辑。改完再替换，省得为了调一个词重跑一次。
 */
...
const props = defineProps<{
  ...
  scopeLabel: string
  /**
   * 是不是「只改格式」那类动作。为真时多两条校验：读者读到的文字必须一字不差，
   * 而标题层级允许变（调层级正是它要干的事）。
   */
  formatOnly?: boolean
  /** 请求还在路上 */
